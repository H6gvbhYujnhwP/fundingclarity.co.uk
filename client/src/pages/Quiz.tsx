      ...
      {!showResult && (
        <div className="container mb-8">
          ...
          <div className="flex justify-between mt-2">
            <span className="text-xs text-warm-white/30" style={{ fontFamily: "var(--font-mono)" }}>
        ...
        <AnimatePresence mode="wait">
          {!showCapture && !showResult && step && (
            ...
            >
              ...
              <p className="text-warm-white/50 mb-8">{step.subtitle}</p>

              <div className="space-y-3">
                {step.options.map((option) => (
                  <motion.button
                    key={option.value}
                    whileHover={{ x: 4, borderColor: "oklch(0.82 0.12 85 / 0.3)" }}
                    whileTap={{ scale: 0.99 }}
                    onClick={() => handleSelect(option.value)}
                    className={`w-full text-left glass-card p-5 rounded-sm flex items-center gap-4 transition-all duration-300 group ${
                  ...
                  >
                    <div
                      className={`w-5 h-5 rounded-full border-2 shrink-0 flex items-center justify-center transition-colors ${
                        answers[step.id] === option.value
                          ? "border-gold bg-gold"
                          : "border-warm-white/20 group-hover:border-gold/40"
                      }`}
                    >
                      {answers[step.id] === option.value && (
                    ...
                    <div>
                      <span className="text-warm-white font-medium block">{option.label}</span>
                      {option.description && (
                        <span className="text-warm-white/40 text-sm">{option.description}</span>
                      )}
                    </div>
                  </motion.button>
                ))}
              ...
              {currentStep > 0 && (
                <button
                  ...
                  className="mt-6 inline-flex items-center gap-2 text-warm-white/40 hover:text-warm-white/70 transition-colors text-sm"
                >
          ...
          {showCapture && !showResult && (
            ...
            >
              ...
              <form onSubmit={handleSubmitCapture} className="space-y-4">
                ...
                <div>
                  <label className="text-sm text-warm-white/60 mb-1 block">Email Address *</label>
                  <input
                    type="email"
                    required
                ...
                <div>
                  <label className="text-sm text-warm-white/60 mb-1 block">Company Name</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-sm text-warm-white/60 mb-1 block">Phone Number</label>
                  <input
                    type="tel"
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    ...
                    className="px-6 py-3 border border-white/10 text-warm-white/60 rounded-sm hover:border-white/20 transition-colors"
                  >
          ...
          {showResult && (
            ...
            >
              {/* Result card */}
              <div className="glass-card p-8 lg:p-12 rounded-sm mb-8">
                <div className="flex items-center gap-3 mb-6">
                  <CheckCircle size={28} className="text-gold" />
                  <span
                    className="text-xs uppercase tracking-[0.3em] text-gold-dim"
                    style={{ fontFamily: "var(--font-mono)" }}
                  >
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <a
                  href="/contact"
                  className="flex-1 inline-flex items-center justify-center gap-3 px-8 py-4 border border-warm-white/20 text-warm-white font-medium rounded-sm hover:border-gold/40 hover:text-gold transition-all duration-300"
                  style={{ fontFamily: "var(--font-display)" }}
                >
                  Send Us a Message
                </a>
              ...
              <p className="text-center text-xs text-warm-white/30 mt-6" style={{ fontFamily: "var(--font-mono)" }}>
